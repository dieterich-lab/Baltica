#!/usr/bin/env python
"""This script is part of leafcutter, but not installled with the package"""
...
def pool_junc_reads(flist, options):
    ...
    chromLst = ["chr%d" % x for x in range(1, 23)] + ['chrX', 'chrY'] + ["%d" % x for x in
                                                                         range(1, 23)] + ['X', 'Y']
    by_chrom = {}
    for libl in flist:
    ...
    for chrom in by_chrom:
        read_ks = [k for k, v in by_chrom[chrom].items() if
                   v >= 3]  # a junction must have at least 3 reads
        read_ks.sort()
