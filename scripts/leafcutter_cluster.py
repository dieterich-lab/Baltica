...
def merge_junctions(options):
    ...
    while len(lsts) > 1:
        ...
        for i in range(0, (old_div(len(lsts), N)) + 1):
            lst = lsts[N * i:N * (i + 1)]
            if len(lst) > 0:
                clst.append(lst)
        lsts = []
...
def get_numers(options):
    ...
    with gzip.open(fname, 'rb') as input_file:
        with gzip.open(fnameout, 'wb') as fout:
            ...
            for l in input_file:
                l = l.decode()
                if first_line:
                    output = " ".join(l.strip().split(" ")[1:]) + '\n'
                    fout.write(output.encode())  # print the sample names
                    first_line = False
                else:
                    l = l.strip()
                    words = l.split(" ")
                    output = words[0] + " " + " ".join([g.split("/")[0] for g in words[1:]]) + '\n'
