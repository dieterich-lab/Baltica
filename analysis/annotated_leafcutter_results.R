...
canonical.usage.rank <- df %>%
  group_by(contrast, cluster) %>%
  group_modify(~ broom::tidy(rank(-.x[[5]] )))
df['canonical.usage.rank'] <- canonical.usage.rank$x
...
genes <- gtf[gtf$type == "gene" & gtf$gene_id %in% multi.exon.genes]
genes.list <- split(genes, genes$gene_id)

# Process annotated introns
...
# Find annotated introns
hits <- findOverlaps(gr, introns, select = 'arbitrary')
overlaps <- pintersect(
  gr[complete.cases(hits)],
  introns[hits[complete.cases(hits)]])
percentOverlap <- width(overlaps) / width(introns[hits[complete.cases(hits)]])

df['annotated'] <- FALSE
df[complete.cases(hits), 'annotated'] <- if_else(
...
gr_on_gtf <- dplyr::select(gr_on_gtf,
                           c("gr", "gene_id", "gene_name", "gene_biotype"))
